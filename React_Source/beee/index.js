...
app.post("/list", (req, res) => {
  ...
  const sqlQuery =
    'SELECT BOARD_NUM, BOARD_TITLE, BOARD_WRITER, BOARD_LOCATION, DATE_FORMAT(BOARD_DATE, "%y-%m-%d") AS BOARD_DATE FROM board_tbl WHERE board_storeId = ? ORDER BY BOARD_NUM DESC LIMIT ?, ?;';
...
app.post("/storelist", (req, res) => {
  ...
  const sqlQuery =
    "SELECT store_id, store_name, store_phone, store_deliveryFee FROM storeInfo_tbl WHERE store_category = ? LIMIT ?, ?;";
...
app.post("/insert", (req, res) => {
  ...
  const sqlQuery =
    "INSERT INTO board_tbl (BOARD_WRITER, BOARD_TITLE, BOARD_CONTENT, BOARD_LOCATION, BOARD_STOREID) VALUES (?, ?, ?, ?, ?);";
  // (?, ?, ?) : [writer, title, content]를 파라미터 값으로 받아온다는 의미
  db.query(
    sqlQuery,
    [writer, title, content, location, storeId],
    (err, result) => {
      // res.send(result);
      res.send("확인");
